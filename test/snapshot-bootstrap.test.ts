...
/**
 * Read the snapshot checkpoint directly from the promoted sqlite database.
 * Storage currently exposes checkpoint writes but not a public checkpoint read
 * helper on the node adapter.
 * @param db - Chain database to read.
 * @param chainID - Chain ID for the checkpoint row.
 * @returns Checkpoint identity, or `undefined` when absent.
 */
function getSnapshotCheckpoint (db: ChainDB, chainID: number) {
  const row = db.$client
    .prepare('SELECT cid, block_height AS blockHeight FROM snapshot_checkpoints WHERE chain_id = ?')
    .get(chainID) as { cid: string, blockHeight: string } | undefined

  if (!row) return undefined
  return {
    cid: row.cid,
    blockHeight: BigInt(`0x${row.blockHeight}`)
  }
}
...
test('valid snapshot state is validated then atomically promoted', async (t) => {
  ...
  const syncState = await createChainStorage(chainDB).getSyncState(NETWORK_CONFIG_ENTRY.chainID)
...
test('existing trusted state skips snapshot bootstrap without validation', async (t) => {
  ...
  await createChainStorage(trustedDB).updateSyncState(NETWORK_CONFIG_ENTRY.chainID, START_HEIGHT)
  ...
  assert.equal(
    (await createChainStorage(unchangedDB).getSyncState(NETWORK_CONFIG_ENTRY.chainID))
      ?.lastBlockHeight,
    START_HEIGHT
  )
...
test('normal engine scan discards an interrupted bootstrap before opening chain.db', async (t) => {
  ...
  await createChainStorage(interruptedDB).updateSyncState(NETWORK_CONFIG_ENTRY.chainID, END_HEIGHT)
  ...
  assert.equal(
    (await createChainStorage(trustedDB).getSyncState(NETWORK_CONFIG_ENTRY.chainID))
      ?.lastBlockHeight,
    END_HEIGHT
  )
  assert.equal(
...
test('a zero-height sync cursor is not trusted and bootstrap proceeds', async (t) => {
  ...
  await createChainStorage(seededDB).updateSyncState(NETWORK_CONFIG_ENTRY.chainID, 0n)
  ...
  assert.equal(
    (await createChainStorage(chainDB).getSyncState(NETWORK_CONFIG_ENTRY.chainID))?.lastBlockHeight,
    END_HEIGHT
  )
  await closeChainDB(chainDB)
})
